...
namespace HeurekaGames.Utils
{
    public static class Heureka_WindowStyler
    {
        ...
        public static void DrawCenteredImage(EditorWindow window, Texture image, string message)
        {
            if (window == null)
                return;

            Rect imageRect = new Rect(
                (window.position.width * 0.5f) - (image.width * 0.5f),
                (window.position.height * 0.5f) - (image.height * 0.5f),
                image.width,
                image.height
            );
            GUI.Box(imageRect, image, GUIStyle.none);
            Vector2 size = GUI.skin.label.CalcSize(new GUIContent(message));
            GUI.Label(new Rect(imageRect.center.x-(size.x/2), imageRect.y+20, size.x, 100), message);
        }
        ...
        public static void DrawCenteredMessage(EditorWindow window, Texture icon, float msgWidth, float msgHeight, string messsage)
        {
            ...
            {
                GUI.Label(new Rect(labelStartPos.x, labelStartPos.y, textWidth, textHeight), lines[i], Heureka_WindowStyler.WhiteLabelStyle);
                labelStartPos.y += 16;
            }
        }
    }
