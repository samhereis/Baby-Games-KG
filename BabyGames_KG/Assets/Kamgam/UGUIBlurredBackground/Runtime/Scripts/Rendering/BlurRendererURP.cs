...
namespace Kamgam.UGUIBlurredBackground
{
    public class BlurRendererURP : IBlurRenderer
    {
        ...
        /// </summary>
        public Material GetMaterial(RenderMode renderMode, BlurredBackgroundImage image)
        {
            // Special case for stacked cameras. If the blur rendering camera and the UI rendering
            // camera are NOT the same camera then we can use the ScreenSpaceRendering pass with the RenderPassEvent.AfterRenderingPostProcessing
            // for camera space canvases too without infinite self rendering problems.
            if (isUsingOverlayCamera(image))
            {
        ...
        protected bool isUsingOverlayCamera(BlurredBackgroundImage image)
        {
            // Special case for stacked cameras. If the blur rendering camera and the UI rendering
            // camera are NOT the same camera then we can use the ScreenSpaceRendering pass with the RenderPassEvent.AfterRenderingPostProcessing
            // for camera space canvases too without infinite self rendering problems.
            ...
            {
                UniversalAdditionalCameraData data = image.canvas.worldCamera.GetUniversalAdditionalCameraData();
