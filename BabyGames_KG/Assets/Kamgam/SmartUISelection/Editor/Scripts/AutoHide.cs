...
namespace Kamgam.SmartUISelection
{
    ...
    // Whenever a canvas is hidden a list of the already hidden objects is made to
    ...
    public static class AutoHide
    {
        ...
        [MenuItem("Tools/Smart UI Selection/Debug/Clear auto-hide user change memory", priority = 401)]
        ...
        private static void AutoHideScreenSpaceOverlayCanvases()
        {
            ...
            {
                // Is enabled?
                if (Settings.enablePlugin && Settings.enableAutoHide)
                {
                    _enabled = true;

                    // Should be checked this frame?
                    float autoHideInterval = Settings.autoHideAlways ? _autoHideCheckIntervaInSec * 2 : _autoHideCheckIntervaInSec;
                    if (EditorApplication.timeSinceStartup - _lastAutoHideCheckTime > autoHideInterval)
                    {
                        ...
                        if (ManagedRootCanvases.Count > 0)
                        {
                            ...
                            if (getValidSceneViewCamera() != null)
                            {
                                ...
                                if (   oldDistance >  Settings.autoHideDistanceThreshold
                                    && newDistance <= Settings.autoHideDistanceThreshold)
                                {
                                    updateNeeded = true;
                                }
                                // Did change to being above the threshold.
                                else if (oldDistance < Settings.autoHideDistanceThreshold
                                      && newDistance >= Settings.autoHideDistanceThreshold)
                                {
                                    updateNeeded = true;
                                }
                                ...
                                shouldBeVisible = newDistance > Settings.autoHideDistanceThreshold;
                            ...
                            if ( Settings.dontAudoHideIfUiSelected
                                && UnityEditor.Selection.activeGameObject != null
                                && UnityEditor.Selection.activeGameObject.transform is RectTransform)
                            {
                                shouldBeVisible = true;
                            }
                            
                            // Don't auto hide if scene view is in 2D mode.
                            if ( Settings.dontAutoHideIfIn2DMode)
                            {
                                bool in2DMode = SceneView.lastActiveSceneView != null && SceneView.lastActiveSceneView.in2DMode;
                                if (in2DMode)
                                    shouldBeVisible = true;
                            }
                            ...
                            if (mouseIsInSceneView == false && Settings.autoHideAlways == false)
                            {
                                shouldBeVisible = true;
                            }
                            ...
                            if (EditorApplication.isPlayingOrWillChangePlaymode && Settings.autoHideDuringPlayback == false)
                            {
                                shouldBeVisible = true;
                            }
        ...
        private static void updateManagedRootCanvasesList(List<ManagedRootCanvas> outRemovedCanvasesToShow)
        {
            ...
            var overlayCanvases = GameObject.FindObjectsByType<Canvas>(FindObjectsInactive.Exclude, FindObjectsSortMode.None)
                                    .Where(canvas => canvas.renderMode == RenderMode.ScreenSpaceOverlay);
#else
            var overlayCanvases = GameObject.FindObjectsOfType<Canvas>()
                                    .Where(canvas => canvas.renderMode == RenderMode.ScreenSpaceOverlay);
#endif
