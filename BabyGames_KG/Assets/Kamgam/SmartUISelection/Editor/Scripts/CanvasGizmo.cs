...
{
    public static class CanvasGizmo
    {
        [DrawGizmo(GizmoType.NotInSelectionHierarchy | GizmoType.NonSelected)]
        static void DrawGizmo(Canvas canvas, GizmoType gizmoType)
        {
            bool executeInPlayModeCheck = !EditorApplication.isPlaying || (Settings.autoHideDuringPlayback && EditorApplication.isPlaying);
            if (Settings.enableAutoHide && Settings.showAutoHideWarningGizmo && executeInPlayModeCheck)
            {
                    ...
                    && canvas.isRootCanvas)
                {
                    ...
                    string distance = string.Format( "({0:0}/{1:0})", AutoHide.GetCamToXYPlaneDistance(), Settings.autoHideDistanceThreshold );

#if UNITY_2019_2_OR_NEWER
                    if (AutoHide.IsVisibilityActiveViaReflection() == false)
                    {
                    ...
                    {
#endif
                        if (AutoHide.GetCamToXYPlaneDistance() <= Settings.autoHideDistanceThreshold)
                        {
                            ...
                            Handles.Label(moveVectorXY(rectCorners[0], 0, 0), text, style);
                            style = GUIStyle.none;
                            style.normal.textColor = Settings.autoHideWarningTextColor;
                            Handles.Label(moveVectorXY(rectCorners[0], -0.1f, 0.1f), text, style);
                        }
                        else
                        {
