...
namespace Kamgam.SmartUISelection
{
    public class SelectionResult
    {
        ...
        public SelectionResult(float distanceToMouse, int hierarchyOrder, Canvas canvas, int canvasIndex, GameObject gameObject)
        {
    ...
    public static class Selection
    {
        ...
        static bool _isDisableKeyPressed = false;   // used to check if key is pressed if "push to skip" is activated.
        static bool _isDisableExcludesKeyPressed = false;   // used to check if key is pressed if "disable exclude lists" is activated.
        static bool _isIgnoreSelectionBaseKeyPressed = false; 
        ...
        {
            if (Settings.enablePlugin && !isMouseInSceneView())
            {
                RectTransformHighlighter.Clear();
            }
                
            // single selections which have not originated from our checks in OnMouseUpInSceneView
            if (Settings.enablePlugin && UnityEditor.Selection.gameObjects.Length == 1 && _mouseDownPos != null)
            {
                ...
                try
                {
                    ...
                    if (isMouseInSceneView())
                    {
                        ...
                        if (UnityEditor.Selection.gameObjects[0].GetComponent<RectTransform>() != null)
                        {
                            ...
                            {
                                // User clicked and moved the mouse a little. Not enough to make a selection rect but
                ...
                catch (Exception e)
                {
                    ...
                    UnityEditor.Selection.objects = selectedObjects;
            ...
            if (Settings.enablePlugin)
            {
                ...
                try
                {
                    ...
                    if (e != null)
                    {
                        ...
                        {
                            if (Event.current.keyCode == Settings.EnableSmartUiKeyCode)
                            {
                                _isEnableKeyPressed = true;
                            }
                            if (Event.current.keyCode == Settings.DisableSmartUIKeyCode)
                            {
                                _isDisableKeyPressed = true;
                            }
                            if (Event.current.keyCode == Settings.DisableExcludeListsKeyCode)
                            {
                                _isDisableExcludesKeyPressed = true;
                            }
                            if (Event.current.keyCode == Settings.SelectionBaseToggleKeyCode)
                            {
                                _isIgnoreSelectionBaseKeyPressed = true;
                            }
                        }
                        else if (e.type == EventType.KeyUp)
                        {
                            ...
                            if (Event.current.keyCode == Settings.SelectionBaseToggleKeyCode)
                            {
                                _isIgnoreSelectionBaseKeyPressed = false;
                            }
                        }
                    }
        ...
        private static void OnMouseUpInSceneView(Vector2 mousePosition, Event mouseEvent = null)
        {
            ...
            if (timeSinceLastMouseUp < Settings.multiClickTimeThreshold
            ...
            if (Settings.pushKeyToUseUiSelection == true && _isEnableKeyPressed == false)
            {
                return;
            }
            
            // ignore events if deactivation key is pressed (only if "push to skip" is activated).
            if (Settings.pushKeyToSkipUISelection && _isDisableKeyPressed)
            {
                return;
            }
            ...
            else
            {
                ...
                canvases = GameObject.FindObjectsByType<Canvas>(FindObjectsInactive.Exclude, FindObjectsSortMode.None).ToList();
#else
            ...
            for (int c = canvases.Count - 1; c >= 0; c--)
            {
                     ...
                     && !IsExcludedByType(canvases[c].transform))
                {
                    ...
                    var rectTransforms = canvases[c].transform.GetComponentsInChildren<RectTransform>(includeInactive: false);
                    for (int i = rectTransforms.Length - 1; i >= 0; i--)
                    {
                        ...
                        if (Settings.limitSelectionToGraphics)
                        {
                            ...
                            if (graphic.color.a <= Settings.alphaMinThreshold)
                            {
                                continue;
                            }
                            ...
                            if (renderer == null || renderer.GetAlpha() <= Settings.alphaMinThreshold)
                            {
                                continue;
                            }

                            // skip if it's part of an invisible (alpha <= threshold) canvas group
                            if (rectTransforms[i].GetComponentsInParent<CanvasGroup>().Any(cg => cg.alpha <= Settings.alphaMinThreshold))
                            {
                                continue;
                            }
                        }
                        ...
                        if (Settings.selectOnlyEditableObjects == true)
                        {
                        ...
                        if (results.FirstOrDefault((r) => r.GameObject == rectTransforms[i].gameObject) == null)
                        {
                            ...
                            results.Add(new SelectionResult(Vector3.Distance(ray.origin, transformHitWorldPos.Value), hierarchyOrder++, rectTransforms[i].GetComponentInParent<Canvas>(), c, objectToAdd));
                        }
            ...
            if ((!Settings.ignoreSelectionBaseAttributes && !_isIgnoreSelectionBaseKeyPressed) || (Settings.ignoreSelectionBaseAttributes && _isIgnoreSelectionBaseKeyPressed))
            {
            ...
            {
                // check if unity default selection has picked something up
                if (UnityEditor.Selection.gameObjects != null)
                {
                    ...
                    // If yes, proceed by ray casting for 3d objects.
                    if (mouseIsInACanvas || UnityEditor.Selection.gameObjects.Any(s => s.GetComponent<RectTransform>() != null))
                    {
                        // search for 3d objects behind the canvas
                        if (Settings.select3dObjectsBehindCanvas)
                        {
                            List<SelectionResult> found3DObjects = new List<SelectionResult>();

                            // raycast colliders
                            if (Settings.select3dColliders == true)
                            {
                                var colliderHits = Physics.RaycastAll(ray, Settings.maxDistanceFor3DSelection);
                                if (colliderHits.Length > 0)
                                {
                                    for (int i = 0; i < colliderHits.Length; i++)
                                    {
                                        ...
                                        {
                                            found3DObjects.Add(new SelectionResult(colliderHits[i].distance, 0, null, int.MaxValue, colliderHits[i].transform.gameObject));
                                        }
                                    }
                                }
                            }
                            ...
                            if (Settings.select3dObjectsByMesh == true)
                            {
                            ...
                            {
#if UNITY_2023_1_OR_NEWER
                                var meshRenderers = Transform.FindObjectsByType<MeshRenderer>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);
#else
                                ...
                                foreach (var renderer in meshRenderers)
                                {
                                    if (found3DObjects.FirstOrDefault(r => r.GameObject == renderer.gameObject) == null) // avoid duplicates
                                    {
                                        ...
                                        {
                                            if (distance < Settings.maxDistanceFor3DSelection)
                                            {
                                                tmpFound3DObjects.Add(new SelectionResult(distance, 0, null, int.MaxValue, renderer.gameObject));
                                            }
                                        }
                                    }
                                }
...
#if UNITY_2023_1_OR_NEWER
                            var spriteRenderers = Transform.FindObjectsByType<SpriteRenderer>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);
#else
                                ...
                                if (found3DObjects.FirstOrDefault(r => r.GameObject == renderer.gameObject) == null) // avoid duplicates
                                {
                                    ...
                                    if (renderer.bounds.IntersectRay(ray, out distance))
                                    {
                                        if (distance < Settings.maxDistanceFor3DSelection)
                                        {
                                            ...
                                            if (Settings.highPrecisionSpriteSelection == false)
                                            {
                                                found3DObjects.Add(new SelectionResult(distance, 0, null, int.MaxValue, renderer.gameObject));
                                            }
                                            else
                                            {
                                    ...
                                    if (meshRenderer != null)
                                    {
                                             ...
                                             )
                                        {
                                            if (meshRenderer.sharedMaterial.HasProperty("_Color")
                                                 && meshRenderer.sharedMaterial.GetColor("_Color").a <= Settings.alphaMinThreshold)
                                            {
                                                return false;
                                            }
                                        }
                            ...
                            else
                            {
                                ...
                                List<GameObject> nativeSelection = new List<GameObject>(UnityEditor.Selection.gameObjects);
                                ...
                                {
                                    selectionChangedBySmartUi = true;
                                    UnityEditor.Selection.objects = nativeSelection.ToArray();
                                }
                            }
                        }
                    }
        ...
        {
            if (Settings.pushKeyToDisableExcludeLists && _isDisableExcludesKeyPressed)
            {
                return false;
            }

            return Settings.excludeByName.Contains(name);
        }
        ...
        public static bool IsExcludedByTag(string tag)
        {
            ...
            return Settings.excludeByTag.Contains(tag);
        }
        ...
        public static bool IsExcludedByType(Transform transform)
        {
            ...
            if (Settings.excludeByType.Count > 0)
            {
                foreach (var excludedType in Settings.excludeByType)
                {
        ...
        private static void setSelection(GameObject[] objects, bool addToSelection)
        {
            ...
            UnityEditor.Selection.objects = _selectionMemory.ToArray();
            
            // Highlight
            if (Settings.highlightSelection)
            {
                RectTransformHighlighter.Highlight(_selectionMemory.ToArray().Where(g => g.transform is RectTransform).Select(g => g.transform as RectTransform).ToList());
            }
        }
        ...
        public static int CompareSelectionResults(SelectionResult x, SelectionResult y)
        {
            ...
            if (x.Canvas != y.Canvas)
            {
                ...
                // If sub-canvases have the same order then compare them by index (if their index differs).
                // Notice: It's important to check isRootCanvas or else if root canvases and sub-canvases are
                // mixed it may not work (sorts wrong if the sub-cavas is a child of the root that this is comparing to).
                if (x.Canvas != null && y.Canvas != null && !x.Canvas.isRootCanvas && !y.Canvas.isRootCanvas)
                {
                    if (x.CanvasIndex > y.CanvasIndex)
                    {
                        return 1;
                    }
                    else if (x.CanvasIndex < y.CanvasIndex)
                    {
                        return -1;
                    }
                }
