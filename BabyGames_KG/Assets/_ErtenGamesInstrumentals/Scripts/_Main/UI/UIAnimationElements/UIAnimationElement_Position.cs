...
namespace UI.UIAnimationElements
{
    public class UIAnimationElement_Position : UIAnimationElement_Base
    {
        ...
        public override void TurnOff(float? duration = null)
        {
            ...
            if (duration.Value == 0)
            {
                _holder.localPosition = _offPosition;
            }
            else
            {
                _holder.DOKill();
                _holder.DOLocalMove(_offPosition, duration.Value).SetEase(_baseSettings.offEase);
            }
#endif
        }
        ...
        public override void TurnOn(float? duration = null)
        {
            ...
            if (duration.Value == 0)
            {
                _holder.localPosition = _onPosition;
            }
            else
            {
                _holder.DOKill();
                _holder.DOLocalMove(_onPosition, duration.Value).SetEase(_baseSettings.onEase);
            }
#endif
        }
    }
