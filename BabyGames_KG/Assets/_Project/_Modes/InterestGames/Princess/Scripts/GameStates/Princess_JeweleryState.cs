...
{
    public class Princess_JeweleryState : Princess_StateBase
    {
        protected override void OnItemSet(Princess_DressBase dressBase)
        {
            ...
            _model.skinCombiner.skinJewilery = dressBase.skinName;
            _model.skinCombiner.Build();

            _model.activityIdentifier.Get<SkeletonAnimation>().AnimationState.ClearTracks();
            _model.activityIdentifier.Get<SkeletonAnimation>().AnimationState.SetAnimation(0, "LightSergaAndOjerelie", false);
        }
    }
}