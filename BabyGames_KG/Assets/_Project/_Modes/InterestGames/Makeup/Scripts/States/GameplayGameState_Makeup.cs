...
{
    public class GameplayGameState_Makeup : IGameState, ISubscribesToEvents
    {
        ...
        private GameplayGameState_Makeup_View _view;

        public GameplayGameState_Makeup(Activity activity)
        {
            _model = new(activity);
            _view = new(_model);
        }
        ...
        public async void Enter()
        {
            ...
            try
            {
                ...
                _model.activityIdentifier = await _model.activity_Identifier_Provider.InstantiaseActivity(_model.activity) as MakeupActivity_Identifier;
                _model.princessController = _model.activityIdentifier.Get<Makeup_Controller>();
                _model._skinCombiner = _model.activityIdentifier.Get<SkinCombiner>();
                await _model.princessController.Initialize(_model);
