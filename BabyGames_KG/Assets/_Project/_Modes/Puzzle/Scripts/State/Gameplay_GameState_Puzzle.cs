using DataClasses;
using GameState;
using Identifiers;
using Interfaces;
using Loggers;
using Services;
using SO;
using System;
using Zenject;

namespace Modes.Puzzle
{
    public class Gameplay_GameState_Puzzle : IGameState, ISubscribesToEvents
    {
        [Inject] private ISceneLoader _sceneLoader;
        [Inject] private ListOfAllScenes_SO _listOfAllScenes;
        [Inject] public IGameStateService gameStateService;

        private Gameplay_GameState_Puzzle_Model _model;
        private Gameplay_GameState_Puzzle_View _view;

        public Gameplay_GameState_Puzzle(Activity activity)
        {
            _model = new(activity);
            _view = new(_model);
        }

        public async void Enter()
        {
            string pictureName = "";

            try
            {
                DiService.Inject(this);

                await _sceneLoader.ShowLoadingLayerAsync();
                await _sceneLoader.LoadSceneAsyncWithTransition(_listOfAllScenes.gameplay_Scene_Puzzle.scene);

                await _model.Initialize();
                await _view.Initialize();

                var activity_Identifier = await _model.activity_Identifier_Provider.InstantiaseActivity(_model.activity) as PuzzleActivity_Identifier;
                activity_Identifier.Construct(_model);
                await activity_Identifier.Initialize();
                SubscribeToEvents();

                _model.replayRequest += Replay;
                _model.goToMainMenuRequest += GoToMainMenu;
                _model.goToNextActivityRequest += GoNextGame;

                await _sceneLoader.HideLoadingLayerAsync();
            }
            catch (Exception ex)
            {
                ex = new Exception($"Could start gameplay - {pictureName}", ex);
                _model.onFatalError.Invoke(ex);
            }
        }

        public void Exit()
        {
            try
            {
                UnsubscribeFromEvents();
            }
            catch (Exception ex)
            {
                CustomLogger.instance?.LogException(ex, "Could not SubscribeToEvents:");
            }
        }

        public void SubscribeToEvents()
        {
            try
            {
                _view.gameplayMenu_Puzzle.backButton.onClick.AddListener(GoToMainMenu);
            }
            catch (Exception ex)
            {
                CustomLogger.instance?.LogException(ex, "Could not SubscribeToEvents:");
            }
        }

        public void UnsubscribeFromEvents()
        {
            try
            {
                _model.replayRequest -= Replay;
                _model.goToMainMenuRequest -= GoToMainMenu;
                _model.goToNextActivityRequest -= GoNextGame;

                _view.gameplayMenu_Puzzle.backButton.onClick.RemoveListener(GoToMainMenu);
            }
            catch (Exception ex)
            {
                CustomLogger.instance?.LogException(ex, "Could not UnsubscribeFromEvents:");
            }
        }

        private void OnFatalError(Exception message)
        {
            CustomLogger.instance?.LogException(message);
        }

        private void Replay()
        {
            MainMenu_GameState.TryOpenGame(_model.activity, gameStateService);
        }

        private void GoToMainMenu()
        {
            UnsubscribeFromEvents();
            gameStateService.ChangeState(new MainMenu_GameState(MainMenu_GameState.OpenSettings.OpenActivitiesMenu));
        }

        private void GoNextGame()
        {
            MainMenu_GameState.TryOpenGame(_model.nextActivity, gameStateService);
        }
    }
}