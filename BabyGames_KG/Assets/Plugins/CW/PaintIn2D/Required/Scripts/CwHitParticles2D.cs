...
namespace PaintIn2D
{
	...
	[HelpURL(CwCommon.HelpUrlPrefix + "CwHitParticles2D")]
	[AddComponentMenu(PaintCore.CwCommon.ComponentHitMenuPrefix + "Hit Particles 2D")]
	public class CwHitParticles2D : MonoBehaviour
	{
		...
		/// <summary>How should the hit data be rotated?</summary>
		...
		/// <summary>Hit events are normally sent to all components attached to the current GameObject, but this setting allows you to override that. This is useful if you want to use multiple <b>CwHitParticles2D</b> components with different settings and results.</summary>
		...
		protected virtual void OnParticleCollision(GameObject hitGameObject)
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				var finalNormal   = rotateTo == RotateType.CollisionNormal ? collision.normal : -collision.velocity;
				var finalRotation = Quaternion.LookRotation(Vector3.forward, -finalNormal);
...
{
	using UnityEditor;
	using TARGET = CwHitParticles2D;

	[CustomEditor(typeof(TARGET))]
	public class CwHitParticles2D_Editor : CwEditor
	{
		protected override void OnInspector()
		{
			...
			Draw("rotateTo", "How should the hit data be rotated?");
			...
			BeginIndent();
				if (Any(tgts, t => t.PressureMode == CwHitParticles2D.PressureType.Constant))
				{
					Draw("pressureConstant", "The pressure value used when PressureMode is set to Constant.", "Constant");
				}
				if (Any(tgts, t => t.PressureMode == CwHitParticles2D.PressureType.Distance))
				{
				...
				else if (Any(tgts, t => t.PressureMode == CwHitParticles2D.PressureType.Speed))
				{
