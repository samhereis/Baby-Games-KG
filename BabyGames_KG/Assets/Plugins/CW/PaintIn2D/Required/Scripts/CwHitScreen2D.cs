...
namespace PaintIn2D
{
	...
	[HelpURL(CwCommon.HelpUrlPrefix + "CwHitScreen2D")]
	[AddComponentMenu(PaintCore.CwCommon.ComponentHitMenuPrefix + "Hit Screen 2D")]
	public class CwHitScreen2D : CwHitScreenBase2D
	{
		...
		public override void HandleFingerUpdate(CwInputManager.Finger finger, bool down, bool up)
		{
			...
			if (RotateTo == RotationType.DrawAngle)
			{
				down = link.State == 2;
				set  = link.State >= 2;
			}
...
{
	using UnityEditor;
	using TARGET = CwHitScreen2D;

	[CanEditMultipleObjects]
	[CustomEditor(typeof(TARGET))]
	public class CwHitScreen2D_Editor : CwHitScreenBase2D_Editor
	{
		protected override void OnInspector()
		{
			...
			var point    = tgt.Emit == CwHitScreenBase2D.EmitType.PointsIn3D;
			var line     = tgt.Emit == CwHitScreenBase2D.EmitType.PointsIn3D && tgt.Connector.ConnectHits == true;
			var triangle = tgt.Emit == CwHitScreenBase2D.EmitType.TrianglesIn3D;
			var coord    = tgt.Emit == CwHitScreenBase2D.EmitType.PointsOnUV;
		...
		protected override void DrawBasic()
		{
			...
			if (Any(tgts, t => t.Frequency == CwHitScreen2D.FrequencyType.PixelInterval || t.Frequency == CwHitScreen2D.FrequencyType.ScaledPixelInterval || t.Frequency == CwHitScreen2D.FrequencyType.TimeInterval))
			{
				BeginIndent();
