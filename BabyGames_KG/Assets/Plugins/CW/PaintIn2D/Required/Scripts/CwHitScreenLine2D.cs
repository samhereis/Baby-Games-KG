...
namespace PaintIn2D
{
	...
	[HelpURL(CwCommon.HelpUrlPrefix + "CwHitScreenLine2D")]
	[AddComponentMenu(PaintCore.CwCommon.ComponentHitMenuPrefix + "Hit Screen Line 2D")]
	public class CwHitScreenLine2D : CwHitScreenBase2D
	{
...
{
	using UnityEditor;
	using TARGET = CwHitScreenLine2D;

	[CanEditMultipleObjects]
	[CustomEditor(typeof(TARGET))]
	public class CwHitScreenLine2D_Editor : CwHitScreenBase2D_Editor
	{
		protected override void OnInspector()
		{
			...
			tgt.Connector.HitCache.Inspector(tgt.gameObject, point: true, line: true);
		}
		...
		protected override void DrawBasic()
		{
			...
			BeginIndent();
				if (Any(tgts, t => t.Frequency == CwHitScreenLine2D.FrequencyType.PixelInterval || t.Frequency == CwHitScreenLine2D.FrequencyType.ScaledPixelInterval || t.Frequency == CwHitScreenLine2D.FrequencyType.StretchedPixelInterval || t.Frequency == CwHitScreenLine2D.FrequencyType.StretchedScaledPixelInterval))
				{
				...
				if (Any(tgts, t => t.Frequency == CwHitScreenLine2D.FrequencyType.Once))
				{
