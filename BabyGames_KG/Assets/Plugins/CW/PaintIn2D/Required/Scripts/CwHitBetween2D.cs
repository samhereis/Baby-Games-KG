...
namespace PaintIn2D
{
	...
	[ExecuteInEditMode]
	[HelpURL(CwCommon.HelpUrlPrefix + "CwHitBetween2D")]
	[AddComponentMenu(PaintCore.CwCommon.ComponentHitMenuPrefix + "Hit Between 2D")]
	public class CwHitBetween2D : MonoBehaviour
	{
...
{
	using UnityEditor;
	using TARGET = CwHitBetween2D;

	[CanEditMultipleObjects]
	[CustomEditor(typeof(TARGET))]
	public class CwHitBetween2D_Editor : CwEditor
	{
		protected override void OnInspector()
		{
			...
			BeginIndent();
				if (Any(tgts, t => t.Orientation == CwHitBetween2D.OrientationType.CameraUp))
				{
					Draw("_camera", "Orient to a specific camera?\nNone = MainCamera.");
				}
				if (Any(tgts, t => t.Orientation == CwHitBetween2D.OrientationType.WorldUp || t.Orientation == CwHitBetween2D.OrientationType.CameraUp))
				{
					Draw("normal", "Which normal should the hit point rotation be based on?");
				}
				if (Any(tgts, t => t.Orientation == CwHitBetween2D.OrientationType.CustomRotation || t.Orientation == CwHitBetween2D.OrientationType.CustomLocalRotation))
				{
			...
			tgt.Connector.HitCache.Inspector(tgt.gameObject, point: true);
		}
	}
}
#endif